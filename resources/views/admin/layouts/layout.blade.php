...
<body>
    ...
    ***********************************-->
    ...
    $("[data-delete-url]").click(function(){
        ...
        var modalTitle = `Delete ${$(this).data("label") && $(this).data("label") != "" ? $(this).data("label") : 'Record'}`;
        if($(this).data("delete-title") && $(this).data("delete-title") !== "")
        {
            modalTitle = $(this).data("delete-title");
        }
        var modalMessage = "Are you sure you want to delete this record? You won't be able to revert this change.";
        if($(this).data("delete-message") && $(this).data("delete-message") !== "")
        {
            modalMessage = $(this).data("delete-message");
        }
        var modalBtnNo = 'No';
        if($(this).data("btn-no") && $(this).data("btn-no") !== "")
        {
            modalBtnNo = $(this).data("btn-no");
        }

        var modalBtnYes = 'Yes';
        if($(this).data("btn-yes") && $(this).data("btn-yes") !== "")
        {
            modalBtnYes = $(this).data("btn-yes");
        }
        var requestType = "false";
        if($(this).data("ajax") && ( $(this).data("ajax") == "true" || $(this).data("ajax") == true) )
        {
            requestType = "true";
        }
        var modalBody = `
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger light" data-dismiss="modal">${modalBtnNo}</button>
                    <button type="button" onclick="deleteRecord($(this),'${requestType}','${dataURL}');" class="btn btn-primary">${modalBtnYes}</button>
                </div>`;
        $("#deleteRecord .modal-content").html(modalBody);
        $("#deleteRecord").modal("show");
    });
    function deleteRecord(button,isAjaxRequest,url)
    {
        $(document).find("#deleteRecordForm").remove();
        $("#deleteRecord").after(`
            <form id="deleteRecordForm" method="POST" action="${url}">
                @csrf
                @method('DELETE')
            </form>
        `);
        var btnOldHtml = button.html();
        if(isAjaxRequest == "true" || isAjaxRequest == true)
        {
            $("#deleteRecordForm").submit(function(e){
                ...
                $.ajax({
                    ...
                    beforeSend: function()
                    {
                        ...
                        button.html(`<span class="text-white"><i class="fa fa-spin fa-spinner"></i> Please wait...</span>`);
                    },
                    success: function(res)
                    {
                        if(res.success)
                        {
                            ...
                            $("#alert-modal .modal-body").html(`<div class="alert alert-success">${res.success}</div>`);
                        }
                        if(res.error)
                        {
                            $("#deleteRecord").modal("hide");
                            $("#alert-modal .modal-body").html(`<div class="alert alert-danger">${res.error}</div>`);

                            button.siblings("button").prop("disabled",false);
                            button.prop("disabled",false);
                            button.html(`${btnOldHtml}`);
                        }
